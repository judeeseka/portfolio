...
function ProjectImage({ project }: { project: Project }) {
  ...
  return (
    <Image
      ...
      className="h-full w-full object-fill rounded-xl group-hover/card:scale-[1.02] transition-transform duration-300"
      alt={project.title}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6 md:gap-8 w-full mt-10">
          {projects.map((project) => (
            <CardContainer key={project.id}>
              <CardBody className="bg-slate-900/80 group/card hover:shadow-xl hover:shadow-emerald-500/5 dark:border-slate-700/60 border border-slate-800 w-full h-auto rounded-2xl p-5 md:p-6 transition-all duration-200 hover:border-slate-600/60">
                <CardItem
                  translateZ={100}
                  className="w-full h-56 md:h-64 mt-0 overflow-hidden rounded-xl flex items-center justify-center bg-slate-800/30"
                >
                  <ProjectImage project={project} />
                </CardItem>
                <CardItem
                  translateZ={60}
                  className="text-lg md:text-xl font-semibold text-white mt-2"
                >
                  {project.title}
                </CardItem>
                <CardItem
                  as="p"
                  translateZ={50}
                  className="text-slate-400 text-sm mt-1.5 line-clamp-2"
                >
                  {project.description}
                </CardItem>
                ...
                >
                  ...
                  <button
                    ...
                    className="text-sm font-medium text-emerald-400 hover:text-emerald-300 underline  cursor-pointer underline-offset-2"
                  >
                    View case study
                  </button>
                </div>
